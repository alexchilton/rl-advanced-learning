...
Worth reading for the exact preprocessing and replay-buffer details the paper elides, but do not try to run it: Torch7 is dead and the repo has had no updates in 9 years.
...
Worth reading for the GAE computation and the conjugate-gradient TRPO step, but Theano is dead; treat it as reference pseudocode, not something to run.
...
Worth reading for PPO's practical tricks (reward/observation normalisation, value clipping) that are not in the paper; TF1 makes it painful to actually run in 2026.
...
Worth reading the original once for the temperature auto-tuning derivation; do not try to run either repo, use a modern PyTorch SAC (e.g. from `sfujim` or CleanRL) instead.
...
Worth reading for the discriminator/generator loop; the rllab dependency makes it effectively unrunnable today.
...
Worth reading and running — short, current PyTorch.
...
Worth reading the `d4rl/` (PyTorch) half; the Atari half drags in an old Dopamine dependency tree that is not worth fighting with.
...
Worth reading and running — small, JAX makes the expectile-regression loss easy to see directly.
...
Worth reading for the observation-normalisation details around the random/predictor networks — they matter more than the paper's headline idea and are easy to get wrong; not worth running given the TF1/MPI dependency stack.
...
Worth reading and running — it is built specifically to isolate each PPO implementation trick one at a time, which is exactly the debugging use case this repo cares about.
...
Worth reading for the reward-predictor-plus-PPO loop structure; treat as historical, do not try to run it.
...
Worth reading for how the reward model and PPO fine-tuning loop are wired together at (then-)scale; archived and TF-based, not meant to run today.
...
Worth reading the constitution prompts themselves; there is no RL training code here, Anthropic did not release one.
...
Worth reading and running — the closed-form loss is a few lines, and this repo is the clearest place to see how it replaces the PPO loop entirely.
...
Worth reading for the group-relative advantage computation directly from the people who proposed it.
...
Worth reading for the length-correlation measurement code specifically — it is the part of this repo most directly reusable for the length-bias demo in Tier 6.
